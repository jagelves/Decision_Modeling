...
Another pattern shown in the graph above is heteroskedasticity (increasing variance), The variance of the series seems to be low from the period of 2016-2018 while significantly higher for the period after. To normalize the variance of the series we can conduct a Box-Cox transformation.
...
```
We can now see how the ensemble fits the test data better than each model individually. Let's confirm the accuracy of the ensemble to the test set by calculating some measures.
