...
1. Recall problem 2 in chapter 6, where you are tasked to forecast the energy consumption of a new ice cream machine. Use the entire data found here: <http://jagelves.github.io/Data/ElectricityBill.csv> to estimate an AR1 model. Graph the ACF and PACF and confirm that an AR1 is appropriate. Check residuals from the model and confirm that they resemble white noise. Create a graph of the series and the forecast for four periods ahead.
...
```{r, fig.show='hold',out.width = '50%'}
#| layout-ncol: 2
Elec_ts %>% ACF(lag_max = 12) %>% autoplot()
Elec_ts %>% PACF(lag_max = 12) %>% autoplot()
```
...
fit %>% augment() %>% PACF(.resid) %>% autoplot()
```
