...
1. The Japanese government has hired you to forecast their total population. The Japanese government is hoping to inform their policy to better allocate resources, and plan for future needs. You can find population data here: <https://jagelves.github.io/Data/UNPOP.csv>
Use the TSLM model with a simple trend and a TSLM model with quadratic trend. Which model has the better BIC? Provide a four year forecast of the best model. Is the population expected to go up or down?
...
```{r}
fit %>% forecast(h=4) %>% filter(.model=="TSLMQ")
```
...
<https://jagelves.github.io/Data/UNPOP.csv> to plot the fertility rate for the US from 1960-2023 and the current replacement rate. What is the two year projected fertility rate when using a simple linear trend? How does this change when using a quadratic trend?
...
USFert %>% model(TSLML=TSLM(Total_Fertility_Rate~trend()),
                 TSLMQ=TSLM(Total_Fertility_Rate~trend()+I(trend()^2))) -> fit
...
3. Grocers Inc. purchases avocados at market price and sells them in their stores for $3 each. Using the avocado price data found here: <https://jagelves.github.io/Data/avocado2020-2023.csv>, calculate the forecasted margin for the company over the next 4 weeks using a simple mean, the NAIVE and simple trend model with seasonality.

<details>
<summary>Suggested Answer</summary>

Forecasts of the price of avocados can be obtained with the following code:
