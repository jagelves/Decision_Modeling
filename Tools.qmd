...
A note on the order above: `extraDistr` and the tidyverse both provide a function called `rdunif()`. Because `extraDistr` is loaded second, `rdunif()` will refer to the `extraDistr` version, which is the one we want.
...
The second cost can be avoided by creating the vector at full size up front and filling it in, which is the disciplined way to write a loop when you must write one.

```{r}
prealloc_time<-system.time({
  Revenue<-numeric(n)
  for (i in 1:n) {
    Revenue[i]<-9.5*rbinom(1,100,0.7)
  }
})["elapsed"]
...
That is a large improvement, and yet the vectorized version is still about `r round(prealloc_time/vector_time)` times faster. The lesson is not that loops are forbidden. It is that in a simulation the loop is almost never buying you anything, and the vectorized version is usually shorter and easier to read as well. **When you can express an operation over a whole vector, do so.**
